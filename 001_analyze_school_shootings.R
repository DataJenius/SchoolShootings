...
plot_summarize_data_by_feature <- function(df,selected_feature, label, title, incidents_only = FALSE) {
  ...
  ggplot(ggplot.data, aes_string(x=selected_feature, y="count", fill="status")) +
    ...
    ylab("Total") +
    labs( title = title,
          subtitle = "All school shootings: July 26, 1764 through March 7, 2018" 
    ) +
    theme(plot.title = element_text(hjust = 0.5),
          plot.subtitle = element_text(hjust = 0.5)) +
    geom_hline(yintercept=mean(df$incidents), linetype="dotted")  
}
...
plot_summarize_data_by_feature_per_incident <- function(df,selected_feature, label, title) {
  ...
  ggplot(ggplot.data, aes_string(x=selected_feature, y="count", group="status", color="status")) +
    ...
    labs( title = title,
          subtitle = "All school shootings: January 1, 1950 through March 7, 2018" 
    ) +
    theme(plot.title = element_text(hjust = 0.5),
          plot.subtitle = element_text(hjust = 0.5))+
    scale_color_manual(
    ...
    geom_hline(yintercept=mean(df$combined.per.incident), linetype="dotted")    
}
...
# Ignore the Wyoming outlier
plot_summarize_data_by_feature_per_incident(by.state[by.state$state!="Wyoming",], "state", "State", "Per Incident by State")
...
full.series <- data.frame(date=as.character(seq(as.POSIXct("1764-07-26"),as.POSIXct("2018-03-08"),by="days")))
...
chart.TimeSeries(plot.incidents.xts,
                 ...
                 main = "School Shootings per Year, Moving Average",
                 lwd = 2,
                 legend.loc = "topleft")
...
chart.TimeSeries(plot.combined.xts,
                 colorset = c("darkorange","darkred","darkblue"),
                 main = "Deaths & Injuries per Incident, Moving Average",
                 lwd = 2,
                 legend.loc = "topleft")
...
ggplot(jojo, aes(x=date,y=combined, fill=decade)) + 
  ...
  labs( title = "The Worst School Shootings since 1950",
        subtitle = "School shootings with at least 10 combined deaths and injuries" 
  ) +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5))
...
ggplot(jojo, aes(x=decade,y=per, fill=decade)) + 
  ...
  guides(fill=guide_legend(title="Year")) +
  labs( title = "Relative Frequency of Worst Incidents",
        subtitle = "Worst incidents as a percentage of all school shootings by decade" 
  ) +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5))
...
state.grades[state.grades$grade=="A",c("grade.score")] <- 96
state.grades[state.grades$grade=="A-",c("grade.score")] <- 92
state.grades[state.grades$grade=="B+",c("grade.score")] <- 89
state.grades[state.grades$grade=="B",c("grade.score")] <- 86
state.grades[state.grades$grade=="C+",c("grade.score")] <- 82
state.grades[state.grades$grade=="C",c("grade.score")] <- 79
state.grades[state.grades$grade=="C-",c("grade.score")] <- 72
state.grades[state.grades$grade=="D",c("grade.score")] <- 66
state.grades[state.grades$grade=="D-",c("grade.score")] <- 62
state.grades[state.grades$grade=="F",c("grade.score")] <- 59
...
ggplot(state.grades, aes(x=grade.score, y=homicide.rate))+
  ...
  scale_x_continuous("Giffords Gun Score (2017)", breaks=c(59,62,66,72,79,82,86,89,92,96), labels=c("F","D-","D","C-","C","C+","B","B+","A-","A")) + 
...
# quick counts
nrow(state.grades[state.grades$incidents==0,c("state","incidents","grade")])
nrow(state.grades[state.grades$incidents>0,c("state","incidents","grade")])
jojo <- state.grades[state.grades$incidents==0,c("state","incidents","grade")]
state.grades[order(state.grades$incidents, decreasing=TRUE),c("state","incidents","grade")]

# D&I v Giffords Gun Score:
